...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    public CameraIntrinsics getCameraIntrinsics() {
        float horizontalAngleView = mCameraEngine.getCameraOptions().getHorizontalViewAngle();

        Size previewSize = mCameraEngine.getPreviewStreamSize(Reference.SENSOR);
        if(previewSize == null) return null;

        float width = previewSize.getWidth();
        float height = previewSize.getHeight();

        double focalLength = (width * 0.5) / Math.tan(horizontalAngleView * 0.5 * PI /180);

        double principalPointX = width * 0.5;
        double principalPointY = height * 0.5;

        return new CameraIntrinsics(focalLength, focalLength, principalPointX, principalPointY);
    }
