...
public class FloatingMarkerTitlesOverlay extends View {
	...
	private void initFMTOverlay() {
		...
		setTextSizeDIP(8);
		setTextPaddingToMarkerDIP(4);
		setMaxFloatingTitlesCount(100);
		setSetMaxNewMarkersCheckPerFrame(10);
		setMaxTextWidthDIP(100);
		setMaxTextHeightDIP(24);
	}
	...
	private void updateCurrentlyDisplayedMarkers(@NonNull final GMFMTGeometryCache _geometryCache) {
		...
		for (final MarkerInfo mi : displayedMarkersList) {
			...
			float xOffset = GMFMTUtils.dipToPixels(getContext(), 7);

			float left = (float) newLocation.x - xOffset;
			float top = (float) newLocation.y - currentArea.height() / 2;
			float right = (float) newLocation.x + textPaddingToMarker + currentArea.width();
			float bottom = (float) newLocation.y + currentArea.height() / 2;

			currentArea.set(left, top, right, bottom);
